...
export const PROVIDERS: Provider[] = [
  ...
  },
  {
    ...
    slot: {
      id: "slot-002",
      starts_at: "2026-08-26T10:00:00+09:00",
      ends_at: "2026-08-26T15:00:00+09:00",
      label: "Wed, Aug 26 · 10:00–15:00",
    },
  },
  {
    id: "provider-003",
...
export function evaluateProviders(
  ...
  return providers.map((provider) => {
    ...
    const deadlineMatches = Date.parse(provider.slot.starts_at) < deadline;
    const matches =
      certificationMatches && budgetMatches && deadlineMatches;

    let reason = "Certified, within budget, and available before the deadline.";
    if (!certificationMatches) {
      reason = "Excluded — required certification is missing.";
    } else if (!deadlineMatches) {
      reason = "Excluded — the first available slot is after the deadline.";
