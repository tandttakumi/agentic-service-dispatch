...
This is the written record of the human-observed native Chrome gate. The committed `artifacts/final-candidate/playwright/**` images remain visibly labeled automated application evidence and are not native-browser proof. The four `artifacts/native-chrome-*.png` files remain immutable public-v1 evidence from 2026-08-27. No screenshot or recording of this August 30 manual gate is included here. The later [August 31 Codex real-agent run](real-agent-verification.md) has its own original page screenshots and verifies 5 → 6 → 5, but not Reset through that agent route. Neither scoped implementation check is a claim of general browser or specification conformance.
